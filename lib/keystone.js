...
// Keystone.prototype.getRequestOptions = this.getRequestOptions(auth_token, path, json_value);
...
Keystone.prototype.getToken = function (username, password, callback) {
	var auth_data = {
		auth : {
			tenantName          : username,
			passwordCredentials : {
	...
	request_options.requireBodyObject = 'access';
	...
	this.request.post(request_options, (error, response, body) => {
		...
		// now we good
		if (typeof callback === 'function') { callback(null, body.access.token.id); }
		callback = undefined;
	});
};
...
Keystone.prototype.getProjectTokenForReal = function (auth_data, callback) {
	var self = this;
...
// make a callback(error, project_authorization) with all of the data on a project and an access token for further calls on it
// NOTE: this is not the admin function that gets project details - you have to do this so I'm not bothering with that
Keystone.prototype.getProjectToken = function (access_token, project_id, callback) {
	var auth_data = {
		auth : {
			identity : {
				methods : [ 'token' ],
...
// make a callback(error, project_authorization) with all of the data on a project and an access token for further calls on it
// NOTE: this is not the admin function that gets project details - you have to do this so I'm not bothering with that
Keystone.prototype.getProjectTokenByName = function (access_token, domain_id, project_name, callback) {
	var auth_data = {
		auth : {
			identity : {
				methods : [
...
Keystone.prototype.listProjects = function (admin_access_token, callback) {
	var self = this;
	var request_options = this.getRequestOptions(admin_access_token, '/v3/projects', true);
...
Keystone.prototype.listUserProjects = function (username, access_token, callback) {
	var self                          = this;
...
Keystone.prototype.getProjectByName = function (admin_access_token, project_name, callback) {
	var self                          = this;
	var request_options               = this.getRequestOptions(admin_access_token, '/v3/projects?name = ' + project_name, true);
...
Keystone.prototype.listRoles = function (project_token, callback) {
	var self                          = this;
...
Keystone.prototype.listRoleAssignments = function (project_token, project_id, callback) {
	var self                          = this;
...
// NOTE: this is only works if the user is authed as an admin or projectAdmin
Keystone.prototype.addRoleAssignment = function (project_token, project_id, entry_id, entry_type, role_id, callback) {
	var request_options = {};
	...
	request_options                = this.getRequestOptions(project_token, '/v3/projects/' + project_id + '/' + entry_type_path + '/' + entry_id + '/roles/' + role_id, true);
	request_options.metricPath     = 'remote-calls.keystone.role-assignments.add';
	request_options.validateStatus = true;

	// Previously passed arg2 response and arg3 body
	this.request.put(request_options, (error) => {
		if (error) {
			if (typeof callback === 'function') { callback(error); }
			callback = undefined;
			return;
...
// NOTE: this is only works if the user is authed as an admin or projectAdmin
Keystone.prototype.removeRoleAssignment = function (project_token, project_id, entry_id, entry_type, role_id, callback) {
	var request_options = {};
	var entry_type_path = 'users';
	...
	// Previously passed arg2 response and arg3 body
	this.request.del(request_options, (error) => {
		if (error) {
			if (typeof callback === 'function') { callback(error); }
			callback = undefined;
			return;
...
Keystone.prototype.listMetaEnvironments = function (auth_token, callback) {
	var self               = this;
	var request_options    = {};
...
Keystone.prototype.listMetaOwningGroups = function (auth_token, callback) {
	var self                = this;
	var request_options     = {};
...
Keystone.prototype.listProjectMeta = function (project_token, project_id, callback) {
	var self            = this;
	var request_options = {};
	var meta_object     = {};

	request_options                   = this.getRequestOptions(project_token, '/v3/projects/' + project_id + '/meta', true);
...
// calls back with callback(error, meta_object)
Keystone.prototype.updateProjectMeta = function (project_token, project_id, new_meta, callback) {
	var self            = this;
	var request_options = {};
	var meta_object     = {};
	var meta_data       = {
	...
	request_options                   = this.getRequestOptions(project_token, '/v3/projects/' + project_id + '/meta', meta_data);
